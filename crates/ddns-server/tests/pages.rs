...
#[tokio::test]
async fn tokens_page_renders_and_creates() {
    let (addr, _broker, cookie, cert) = setup_admin().await;

    // Authed GET /tokens → 200, contains table + form.
    let resp = http1(
        addr,
        &cert,
        "tunnel.example.com",
        &page_req(&cookie, "/tokens"),
    )
    .await;
    assert_eq!(status_code(&resp), 200, "GET /tokens should return 200");
    assert!(
        resp.contains("No tokens yet") || resp.contains("<table"),
        "tokens page should contain empty message or table"
    );
    assert!(
        resp.contains("Create Token"),
        "tokens page should contain create form"
    );

    // Create a token via the form (POST /tokens with form data).
    let create_body = form_body(&[
        ("name", "my-test-token"),
        ("max_sessions", "2"),
        ("max_streams", "32"),
    ...
    .await;
    assert_eq!(
        status_code(&create_resp),
        201,
        "POST /tokens should return 201"
    );
    assert!(
        create_resp.contains("Copy this secret"),
        "secret box should appear"
    );
    assert!(
        create_resp.contains("copy-btn"),
        "copy button should appear"
    );

    // Now GET /tokens again and check the token name appears.
    let resp2 = http1(
        addr,
        &cert,
        "tunnel.example.com",
    ...
    .await;
    assert_eq!(status_code(&resp2), 200);
    assert!(
        resp2.contains("my-test-token"),
        "created token name should appear in table"
    );
}
...
#[tokio::test]
async fn settings_defaults_card_renders_and_persists() {
    let (addr, _broker, cookie, cert) = setup_admin().await;

    // Card renders the current (default) values.
    let resp = http1(
        addr,
        &cert,
        "tunnel.example.com",
    ...
    for needle in [
        ...
        assert!(resp.contains(needle), "missing {needle} in defaults card");
    }

    // Reject a negative value (validate >= 0).
    let body = form_body(&[
        ("max_sessions", "-1"),
        ("max_streams", "7"),
        ("max_bytes", "100"),
    ...
    let resp = http1(
        ...
        &cert,
        "tunnel.example.com",
        &form_req(&cookie, "/settings/defaults", &body),
    )
    .await;
    assert_eq!(status_code(&resp), 400, "negative rejected, got:\n{resp}");

    // Save distinctive values (heartbeat set).
    let body = form_body(&[
        ("max_sessions", "5"),
        ("max_streams", "7"),
        ("max_bytes", "100"),
    ...
    assert!(
        ...
        "max_sessions persisted:\n{resp}"
    );
    assert!(
        resp.contains("value=\"7\""),
        "max_streams persisted:\n{resp}"
    );
    assert!(
        resp.contains("value=\"100\""),
        "max_bytes persisted:\n{resp}"
    );
    assert!(resp.contains("value=\"60\""), "ttl_secs persisted:\n{resp}");
    assert!(
        resp.contains("value=\"30000\""),
        "heartbeat_ms persisted:\n{resp}"
    );
}

#[tokio::test]
async fn settings_defaults_heartbeat_empty_clears() {
    let (addr, _broker, cookie, cert) = setup_admin().await;

    // Set heartbeat, then clear it with an empty field.
    let body = form_body(&[
        ("max_sessions", "2"),
        ("max_streams", "32"),
        ("max_bytes", "2147483648"),
    ...
    .await;
    assert_eq!(status_code(&resp), 303, "set heartbeat, got:\n{resp}");
    ...
    .await;
    assert_eq!(status_code(&resp), 303, "clear heartbeat, got:\n{resp}");

    // Empty heartbeat renders as an empty input again.
    let resp = http1(
        addr,
        &cert,
        "tunnel.example.com",
    ...
    assert_eq!(status_code(&resp), 200);
    assert!(
        resp.contains("name=\"heartbeat_ms\"") && !resp.contains("value=\"15000\""),
        "heartbeat cleared:\n{resp}"
    );
}
